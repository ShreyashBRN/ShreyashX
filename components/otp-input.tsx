...
function accentFill(theme: ThemeMode): React.CSSProperties {
  return theme === "dark"
    ? {
        ...
        boxShadow:
          "0 6px 16px -8px rgba(124,58,237,0.55), 0 0 0 1px rgba(255,255,255,0.08) inset",
      }
    : {
        background: "linear-gradient(155deg, #9b8cf2 0%, #6d4cec 50%, #5432d6 100%)",
        boxShadow:
          "0 6px 14px -8px rgba(84,50,214,0.35), 0 0 0 1px rgba(255,255,255,0.30) inset",
      };
}
...
function slotStyle(theme: ThemeMode, tinted: boolean): React.CSSProperties {
  if (theme === "dark") {
    return tinted
      ? { background: "rgba(139,92,246,0.10)", border: "1px solid rgba(139,92,246,0.22)", color: "#ede9fe" }
      : { background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.10)", color: "#ede9fe" };
  }
  return tinted
    ? { background: "rgba(124,92,240,0.06)", border: "1px solid rgba(124,92,240,0.18)", color: "#241b3d" }
    : { background: "rgba(255,255,255,0.6)", border: "1px solid rgba(15,23,42,0.12)", color: "#241b3d" };
}
...
function dividerColor(theme: ThemeMode) {
  return theme === "dark" ? "rgba(255,255,255,0.10)" : "rgba(15,23,42,0.08)";
}
...
const SLOT_SIZE = "w-[clamp(38px,10vw,54px)] h-[clamp(38px,10vw,54px)]";
const GAP = "gap-[clamp(10px,3vw,18px)]";
const FONT_SIZE = "text-[clamp(16px,4vw,19px)]";
...
function OtpCore({
  ...
  // Capsule segment: inset within the segment, small radius (not a pill).
  ...
  const commonInputBase =
    "flex-none rounded-lg text-center font-semibold tracking-wide outline-none transition-colors duration-200 relative z-10 caret-current disabled:opacity-40";

  /* -------------------------- CLASSIC (square slots) -------------------------- */
  if (variant === "classic") {
    return (
      <div ref={containerRef} className={cn("relative inline-flex", GAP)}>
        <motion.div
        ...
        {Array.from({ length }).map((_, i) => {
          ...
          return (
            <SlotInput
              ...
              className={cn(commonInputBase, SLOT_SIZE, FONT_SIZE)}
              style={{
                ...base,
    ...
    return (
      <div ref={containerRef} className={cn("relative inline-flex", GAP)}>
        ...
        {Array.from({ length }).map((_, i) => {
          const active = i === activeIndex;
          return (
            <div key={i} className={cn("relative flex flex-col items-center", SLOT_SIZE)}>
              <SlotInput
                index={i}
                length={length}
                digits={digits}
                ...
                onPasteSlot={handlePaste}
                className={cn(
                  "w-full h-full bg-transparent text-center font-semibold outline-none transition-colors duration-200 relative z-10 pb-2 disabled:opacity-40",
                  FONT_SIZE
                )}
                style={{ color: active ? underlineColor(themeMode, true) : slotStyle(themeMode, false).color }}
              />
              <span
                className="absolute bottom-0 h-[2px] w-[62%] rounded-full transition-colors duration-200"
      ...
      >
        ...
        {Array.from({ length }).map((_, i) => {
          ...
          return (
            <div
              key={i}
              className={cn(SLOT_SIZE, "relative flex items-center justify-center")}
            ...
            >
              <SlotInput
                ...
                onPasteSlot={handlePaste}
                className={cn(
                  "w-full h-full bg-transparent text-center font-semibold outline-none transition-colors duration-200 relative z-10 disabled:opacity-40",
                  FONT_SIZE
                )}
                style={{ color: active ? "#ffffff" : themeMode === "dark" ? "#ede9fe" : "#241b3d" }}
              />
            </div>
          );
        })}
...
export function OtpInput({
  ...
  return (
    <div className={cn("grid w-full grid-cols-1 gap-8 md:grid-cols-2 md:gap-10", className)}>
      {variants.map((v) => (
        <div key={v} className="flex items-center justify-center p-2">
