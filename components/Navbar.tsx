...
export default function Navbar() {
  return (
    <header className="fixed top-0 z-50 w-full border-b border-black/5 bg-[#f6f4ef]/80 backdrop-blur-md">
      <Container className="flex h-16 items-center justify-between">
        {/* Logo: desktop only now. On mobile the header is just the nav
            links (below) — no logo, no hamburger, no fullscreen overlay —
            mirroring ComponentsNavbar's mobile treatment. */}
        <Link
          href="/"
          className="hidden text-[1.05rem] font-inter font-bold tracking-[-0.04em] text-[#141816] transition-colors duration-300 hover:text-[#0d7377] md:inline-block"
        >
          Shreyash Bagade
        </Link>

        {/* Mobile: left-aligned, matching ComponentsNavbar's layout exactly
            (no centering — it just sits at the header's flex-start since
            the logo beside it is hidden). Desktop (md+) is unchanged. */}
        <nav className="flex items-center gap-6 text-[15px] font-medium sm:gap-8">
          <NavLink href="/">Home</NavLink>
          <NavLink href="/blog">Blog</NavLink>
          <NavLink href="/components">Components</NavLink>
        </nav>
